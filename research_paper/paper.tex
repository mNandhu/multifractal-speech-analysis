  ...
  Naive random splits produced unrealistically high scores in early experiments, confirming that speaker leakage can strongly inflate apparent performance. All results reported below therefore use the speaker-independent evaluation protocol.

  %--------------------------------------------------------------
  \subsection{Speaker-Independent Evaluation}
  %--------------------------------------------------------------

  To obtain more reliable performance estimates, all subsequent experiments employ speaker-independent cross-validation. Only the sustained vowel token \texttt{a\_n} is used to ensure consistency across speakers and to minimise variability introduced by different speech tasks.
  ...
  In contrast, adding NeuroKit2 features yields only marginal improvement, suggesting that most of the useful complexity-related information is already captured by the multifractal descriptors. This result is notable given that MFDFA produces only 12 descriptors yet approaches configurations containing over 100 additional parameters. For resource-constrained or real-time screening scenarios, the MFDFA-only configuration therefore offers a compelling trade-off between feature dimensionality and predictive performance.
  ...
  Further experiments show that while binary pathology detection is rather manageable, fine-grained disease classification appears to be more difficult due to acoustic overlap between diseases. In conclusion, this study indicates that multifractal descriptors provide a promising complement to traditional spectral features for automated voice-pathology screening.

  Several limitations of the present study should be acknowledged. Experiments are restricted to the sustained vowel /a/ at normal pitch; whether multifractal descriptors generalise to other phonation tasks or continuous speech remains to be established. The disease-group evaluation also merges heterogeneous pathologies into two broad clinical categories, which may obscure within-group variation. Finally, while speaker-independent cross-validation mitigates data leakage, external validation on an independent cohort would be required to confirm generalisation to clinical populations outside the SVD.
